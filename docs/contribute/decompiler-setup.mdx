...
1. Go to ghidra.decomp.dev and link your Discord account.
...
2. Download the latest [RootCubed Ghidra](https://github.com/encounter/ghidra-ci/releases) build. Make sure you download `GameCubeLoader` artifact as well.
...
You will want to checkout `gcn_usa` for working on decompilation. We also highly recommend checking-out `tp_shield_chn_debug_new` as it offers much more insight into the code through debug strings, less aggressive function inlining, and extra debug tools.
